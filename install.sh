...
SetupGPU () {
  ...
  envConfigPath="$REPO_DIR/hypr/configs/env.lua"
  aqDrmDevicesPattern='^[[:space:]]*hl[.]env[(][[:space:]]*"AQ_DRM_DEVICES"[[:space:]]*,'
  ...
  if ! grep -Eq "$aqDrmDevicesPattern" "$envConfigPath"; then
    logInfo 'No AQ_DRM_DEVICES override found in env.lua'
    return
  fi

  logInfo 'Removing AQ_DRM_DEVICES override from env.lua because its system specific.'
  tempFile=$(mktemp)

  awk '!match($0, /^[[:space:]]*hl[.]env[(][[:space:]]*"AQ_DRM_DEVICES"[[:space:]]*,/) { print }' "$envConfigPath" > "$tempFile"
...
SetupMonitors () {
  ...
  printf '%s\n' "$monitorConfig" > "$REPO_DIR/hypr/configs/monitors.lua"

  logInfo 'Updated hypr/configs/monitors.lua'
}
...
BuildAutoMonitorConfiguration () {
  ...
  {
    printf '%s\n\n' '-- Auto-generated by install.sh using `hyprctl monitors -j`'
...
BuildCustomMonitorConfiguration () {
  ...
  {
    printf '%s\n\n' '-- Auto-generated by install.sh using `hyprctl monitors -j` with user-provided positions'

    while IFS=$'\t' read -r output width height refresh <&3; do
      [[ -z $output ]] && continue
...
PromptForMonitorCoordinate () {
  ...
  local axis=$2
  local resultName=$3
  local -n resultRef=$resultName

  while true; do
    promptForValue "Enter ${axis} position for monitor ${output}: " "$resultName" >&2

    if [[ $resultRef =~ ^-?[0-9]+$ ]]; then
      return
    fi

    logError 'Please enter an integer value.' >&2
  done
}
...
PromptForMonitorScale () {
  ...
  while true; do
    promptForValue "Enter scale for monitor ${output} [default: 1]: " "$resultName" >&2
    ...
    logError 'Please enter a numeric scale value.' >&2
  done
}
